...
df.insert(1, 'util', '', True)
print('top df::: ', df.to_string)


# insert cell value ^
df = df.sort_index().reset_index(drop=True)
print(df.to_string)
print('FIRST df: ', tabulate(df, headers='keys', tablefmt='psql'))
#
# geospatial section
...
print('DF 444: ', tabulate(df4, headers='keys', tablefmt='psql'))
#All these sections could master_nodeefully be rolled into one function

# output file name -v
name = 'master_node_DQ_nodes_checklist27.xlsx'

#Add issue GBIF.org urls
api_issue = 'https://www.gbif.org/occurrence/search?issue={}&publishing_country=DK&advanced=1'
url_dict = {}
for j in issues_list:
    gbif_issue_url = api_issue.format(j)
    print(gbif_issue_url)
    url_dict[j] = gbif_issue_url
print('final url_dict', url_dict)

df_urls = pandas.DataFrame.from_dict(url_dict, orient='index')
print(tabulate(df_urls, tablefmt='psql', headers='keys'))
df_list = [df4, df_urls]
